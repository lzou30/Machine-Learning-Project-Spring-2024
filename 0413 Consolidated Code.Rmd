...
debt_clean <- as.data.frame(county_debt[, -c(2,3)])
...
corr_matrix <- cor(debt_clean, use = "pairwise.complete.obs")
...
debt <- debt_clean
means <- apply(debt,2,mean)
sds <- apply(debt,2,sd)
nor <- scale(debt,center=means,scale=sds)
