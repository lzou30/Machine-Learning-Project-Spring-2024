...
```{r}
# Load packages
pacman::p_load(tidyverse, ggplot2, scales, glmnet, pls, randomForest, rpart, DescTools, dplyr, readxl)
...
#group1.1$ZIP.code <- sprintf("%05d", group1$ZIP.code)
...
chisq.test(group1.3$Tags, group1.3$missing_zip) # Does not meet assumptions
...
# Tags
table(group1.3$Tags, group1.3$missing_zip)
fisher.test(group1.3$Tags, group1.3$missing_zip) # Does not meet assumptions with Chi-sq, so use Fisher Test instead; no difference between the two groups
...
# use this for computational efficiency
# To use RFinput, make all wrong zipz NAs before fixing and then run this
```
...
county_debt$fips <- county_debt$`County FIPS`
group1.7 <- merge(group1.6, county_debt, by = c("fips"), all.x = TRUE)
```

```{r}
# Read in Census demographic data
census <- read.csv("C:/Users/lzou7/OneDrive/Desktop/GWU/ML in R/Project/cc-est2022-all.csv")

# Combining the State and County FIPs
#making sure all of the leading zero and cushion zeros are included
census$county_fips <- sprintf("%03d", census$COUNTY)
census$state_fips <- sprintf("%02d", census$STATE)
# Final fips code
census$fips <- paste(census$state_fips, census$county_fips, sep = "")
...
# Creating features
features <- 
# unique(fips) observations for feature measures
```