...
#perform a chi-squared to test for any significant associations between whether debt relief was granted and observations with missing zipcodes
...
# Imputing by using the mode for zip codes by state means that the data will be biased toward counties that appear more in the data. 
```

## d. Please indicate any limitations or biases your imputation has on the analysis?
  Imputing by using the mode for zip codes by state means that the data will be biased toward counties that appear more in the data. 
Large counties or counties with large cities thus appear more in the data, and our subsequent models and analysis will be biased toward denser populations.

## i. If I replace all values with the state capital zip code, how would that impact my analysis?
  If all of the zip code values where replace with state capital zip codes, model analysis would be biased toward heavily urban areas. Also since most cities tend to hold diverse populations, the data may appear to cover demographics more equally when it actually does not. This improper representation of demographics could lead to discrimination in debt relief grants.
...
# Creating percentage variables to indicate bias
census4 <- census4 %>%
...
# Create a flag for service members and older Americans from the Tags variable
...
```{r}
# Random forest imputation for missing data in the Urban Institute data
# Make a subset for imputation of county debt variables
...
group1.9 <- group1.8[,-c(24:50)] # drop unneeded debt variables 
...
# make sure all data types are correct for clustering
z[,1]<- as.factor(z[,1])
z[,2]<- as.factor(z[,2])
...
abline(0, 1, col = "red")
#we ran a LOGIT with ridge regularization to solve the multicollinearity issue we had 
...
# Run an interaction plot to see what pct_women interacts with 
...
boost.F1.2 <- F1_Score(ifelse(boost.predict2 > 0.5, 1, 0), ytest) # 0.8850649, it imporved by about 16% with the optimal parameters

# Get importance matrix of new model with correct parameters
...
test_all1.3 <- test_all1.2 %>%
  mutate(Submitted.via = case_when(Submitted.via == "Web Referral" ~ "Referral",
            ...
            Submitted.via == "Postal mail" ~ "Postal mail",
            Submitted.via == "Phone" ~ "Phone",
            Submitted.via == "Fax" ~ "Fax")) # Recode Web Referral to Referral because it has a small number of observations
...
test_all1.6 <- merge(test_all1.5, zip_fips_states, by.x = c("imp_zip", "State"), by.y = c("ZIP", "STATE")) # merge datasets
...
## c. Which has the best fit?
  ...
  Based on accuracy and F1 score, XGBoost does the best, but since the goal of these models is the predict debt relief, we want to be able to explain the reasoning behind debt relief application acceptance versus rejection. Thus, our random forest model with the second best accuracy rate and F1 score is more optimal for our goal as we can expplain and interpret its outcomes.
...
# 12. After picking the best model, drop one variable. The dummy variable for Older American you made from the original data set tags and rerun. Does the aggregated older American percentages from the Census data provide the same level of details and predictive accuracy as the micro data provided by the original dataset? Do you think these results are representative of other demographic characteristics.
  After removing the binary variable for "Older American" from the original data set and rerunning the model, the accuracy scores and F1 scores did not decrease significantly. This suggests that the aggregated older American percentages from the Census data do inform the model sufficiently to have similar predictive accuracy. The aggregated percentages of ethnicity and race, however, make it difficult to judge if these results are adequately representative of other demographic characteristics. 

```{r}
# Remove dummy for Older American inn train and test datasets 
