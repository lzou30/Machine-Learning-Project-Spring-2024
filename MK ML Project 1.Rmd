...
# Load packages
pacman::p_load(tidyverse, ggplot2, scales, glmnet, pls, randomForest, rpart, DescTools, readxl, corrplot, ggcorrplot, factoextra, ggfortify, clustMixType, wesanderson, caret, rpart.plot, xgboost, reshape2, MLmetrics)
...
group2.1 <- group2.0[, -c(1:3, 11:87, 98:99)]
...
# New dataset with the variables stepwise 
group2.2 <- group2.1 %>%
...
lm2 <- lm(as.numeric(as.character(relief)) ~., data = train.data)
summary(lm2)
y_pred <- predict(lm2, test.data)
y_pred <- round(y_pred, 0)
table(y_pred, test.data$relief)
F1_lm <- F1_Score(y_pred, test.data$relief) # 0.925255
...
cv_model <- cv.glmnet(x[train,], y[train], alpha = 0, family = "binomial")
lambda <- cv_model$lambda.min

probabilities <- predict(ridge.logit, newx = x[test,], s = lambda, type = "response")
y_pred <- ifelse(probabilities > 0.5, 1, 0)
table(y_pred, y[test])
F1_ridge_logit <- F1_Score(y_pred, y[test]) # 0.9252302

### CART/Regression Tree
set.seed(182654)
relief.tree <- rpart(relief ~., data = train.data, method = "class", cp = 0.0001)

printcp(relief.tree)
rpart.plot(relief.tree) # plot tree
relief.tree$variable.importance
y_pred <- predict(relief.tree, test.data, type = "class") # plot variable importance
c_matrix <- table(test.data$relief, y_pred)
misclass_rate_tree <- (938+2899)/(19060+938+2899+332) # 0.1651815
F1_tree <- F1_Score(y_pred, test.data$relief) # 0.9085492

### Random Forest
# Run random forest
set.seed(182654)
relief.rf <- randomForest(relief ~., data = train.data,
                        importance = TRUE, ntree = 500)
relief.rf
plot(relief.rf)
pred.relief.rf <- predict(relief.rf, test.data, method = "class")
table(test.data$relief, pred.relief.rf)
misclass_rate_rf <- (3080 + 365)/(3080 + 365 + 151 + 19633) # 0.148306
F1_rf <- F1_Score(pred.relief.rf, test.data$relief) # 0.9193416

# XGBOOST
## Create train and test data set.
set.seed(193456)
train <- sample(1:nrow(group2.2), nrow(group2.2)/2)
boost.train <- group2.2[train,]
boost.test <- group2.2[-train,]

# Create model matrices
xtrain <- sparse.model.matrix(relief ~ .-1, data = boost.train) 
ytrain <- as.array(boost.train$relief) 
xtest <- sparse.model.matrix(relief ~ .-1, data = boost.test) 
ytest <- as.array(boost.test$relief)
...
confusion_matrix <- table(Actual = ytest, Predicted = ifelse(y_pred > 0.5, 1, 0))
misclass_rate <- (182+3121)/(19842 + 182 + 3121 + 84) # 0.1421929

F1_XGB <- F1_Score(ifelse(y_pred > 0.5, 1, 0), ytest) # 0.9231628


### Perform a grid search for the proper parameters
# Grid searching hyperparameter values 
# = parameters = # 
# = eta candidates = # 
...
pred_eta = matrix(NA,nrow(boost.test), length(eta))
...
# Find RMSE
(RMSE_eta = sqrt(colMeans((as.numeric(ytest)-pred_eta)^2))) # 0.05
...
pred_cs = matrix(NA,nrow(boost.test), length(cs)) 
...
# Find RMSE
(RMSE_cs = sqrt(colMeans((as.numeric(ytest)-pred_cs)^2))) # 1/3

####################################### 
## Max Depth 
set.seed(1284654) 
conv_md=matrix(NA,500,length(md)) 
pred_md=matrix(NA,nrow(boost.test),length(md)) 
...
# Find RMSE
(RMSE_md=sqrt(colMeans((as.numeric(ytest)-pred_md)^2))) # 2

##########################
## sub_sample 
set.seed(1284654) 
conv_ss=matrix(NA,500,length(ss)) 
pred_ss=matrix(NA,nrow(boost.test),length(ss)) 
...
# Find RMSE
(RMSE_ss=sqrt(colMeans((as.numeric(ytest)-pred_ss)^2))) # 1
...
pred_mcw = matrix(NA,nrow(boost.test), length(mcw)) 
colnames(conv_mcw) = colnames(pred_mcw) = mcw 
for(i in 1:length(mcw)){ 
  params = list(eta = 1, colsample_bylevel=1/3, 
                subsample = 1, max_depth = 2, min_child_weight = mcw[i], gamma = 0) 
  xgb = xgboost(xtrain, label = ytrain, nrounds = 500, params = params) 
...
# Find RMSE
(RMSE_mcw = sqrt(colMeans((as.numeric(ytest)-pred_mcw)^2))) # 400
...
pred_gamma = matrix(NA,nrow(boost.test), length(gamma)) 
colnames(conv_gamma) = colnames(pred_gamma) = gamma 
for(i in 1:length(gamma)){ 
  params = list(eta = 1, colsample_bylevel=1/3, 
                subsample = 1, max_depth = 2, min_child_weight = 10, 
                gamma = gamma[i]) 
  xgb = xgboost(xtrain, label = ytrain, nrounds = 500, params = params) 
...
# Find RMSE
(RMSE_gamma = sqrt(colMeans((as.numeric(ytest)-pred_gamma)^2))) # 1

### Running the model with the correct parameters
# Setting hyperparameter values
# Had to play around with the interaction between the values to improve classification rate
set.seed(194564)
params = list(eta = 1, colsample_bylevel = 1/3, subsample = 1 , max_depth = 2, min_child_weight = 100, gamma = 1)
xgb.train <- xgboost(data = xtrain, label = as.numeric(ytrain) - 1, params = params,
                     nrounds = 500, objective = "binary:logistic")

y_pred2 <- predict(xgb.train, xtest)
confusion_matrix2 <- table(Actual = ytest, Predicted = ifelse(y_pred2 > 0.5, 1, 0))
confusion_matrix2
misclass_rate2 <-  (3166+46)/23229 # 0.1382754 (eta = 1, colsample_bylevel = 1/3, subsample = 1 , max_depth = 2, min_child_weight = 100, gamma = 1)
...
# Get F1 score
F1_XGB_opt <- F1_Score(ifelse(y_pred2 > 0.5, 1, 0), ytest) # 0.925593
