...
group2.2 <- group2.1 %>%
  select(c(relief, Sub.product, Issue, Tags, Consumer.consent.provided., Submitted.via, Timely.response., pct_blk, pct_hisp, pct_women, AldrA, CcddrA, CcddrCoc, Sopoc, PC1, cluster4, year)) %>%
...
set.seed()
relief.tree <- rpart(relief ~., data = train.data, minsplit = 10, minbucket = 5, method = "class")
printcp(relief.tree)
rpart.plot(relief.tree)
relief.tree$variable.importance

y_pred <- predict(relief.tree, test.data)
...
misclass_rate_rf <- (3093 + 361)/(3093 + 361 + 19637 + 138) # 0.1486934
...
xgboost_test = xgb.DMatrix(data=xtest, label=ytest)
...
misclass_rate <- (198+3114)/(19826 + 198 + 91 + 3114) # 0.1425804
...
for(i in 1:length(mcw)){ 
  params = list(eta = 0.1, colsample_bylevel=2/3, 
                subsample = 1, max_depth = 6, min_child_weight = mcw[i], gamma = 0) 
  xgb = xgboost(xtrain, label = ytrain, nrounds = 500, params = params) 
...
params = list(eta = .05, colsample_bylevel = 1/3, subsample = 1 , max_depth = 2, min_child_weight = 400, gamma = 1)
ytrain_numeric <- as.numeric(as.character(ytrain))
xgb.train <- xgboost(data = xtrain, label = ytrain, params = params,
                     nrounds = 500)

y_pred <- predict(xgb.train, xtest)
confusion_matrix2 <- table(Actual = ytest, Predicted = ifelse(y_pred > 0.5, 1, 0))
misclass_rate2 <- (3105+276)/(3105+276+19748+100) # 0.1455508
