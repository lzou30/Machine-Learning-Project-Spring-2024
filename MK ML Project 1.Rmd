...
# Calculate the percent missing for each variable
num_missing <- colSums(is.na(debt_subset_rf2))
pct_missing <- as.data.frame(num_missing/nrow(debt_subset_rf2))
...
# Oversample the data and create a test and train
...
ridge.spec <- 6287/(6287 + 3687) # 0.6303389
...
lm.spec <- 6289/(6289+3685) # 0.6305394
...
tree.spec <- 5087/(5087 + 4887) # 0.5100261
...
rf.spec <- 9701/(9701 + 273) # 0.9726288
...
multi_import <- plot_multi_way_importance(importance_frame, size_measure = "no_of_nodes", no_of_labels = 5)
multi_import
...
plot_min_depth_interactions(interactions_frame[which(interactions_frame$variable == "pct_women"),]) # plot what pct_women interacts with, specifically
...
boost.spec <- 9594/(9594 + 380) # 0.9619009
