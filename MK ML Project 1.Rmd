...
# Load packages
pacman::p_load(tidyverse, ggplot2, scales, glmnet, pls, randomForest, rpart, DescTools, readxl, corrplot, ggcorrplot, factoextra, ggfortify, clustMixType, wesanderson, caret)
...
group1.9$cluster3 <- as.factor(group1.9$cluster3)
...
group2.0 <- group1.9 %>%
  select(-c(State, ZIP.code, Consumer.disputed., Sopoc, SoslhwsldidA, old_service_mem, medical_debt, Consumer.complaint.narrative, Complaint.ID,
            Product, COUNTYNAME, STCOUNTYFP, AGEGRP, Company, Date.sent.to.company))
...
group2.1 <- group2.0[, -c(3, 14:90, 102:103)]
...
```{r, results = FALSE}
# LM Stepwise
lm <- lm(relief ~., data = group2.1[,-c(1:2)])
lm.step <- step(lm, direction = "both") # AIC = -100872.9

# New dataset with the variables stepwise chose
group2.2 <- group2.1 %>%
  select(c(relief, Sub.product, Issue, Sub.issue, Company.public.response, Tags, Consumer.consent.provided., Submitted.via, Timely.response., pct_asian, pct_women, AldrCoc, CcddrA, CcddrCoc, MccddWc, PC1, year))
...
glm.logit <- glm(relief ~ ., data = train.data, family = binomial(link = "logit"))
...
# LASSO
# set.seed(182654)
# x <- model.matrix(step_kept_vars$relief ~ ., step_kept_vars)
# y <- step_kept_vars$relief
...
# 
# # Run model
# lasso.glm <- glmnet(x[train,], y[train], alpha = 1, lambda = grid, thresh = 1e-12)
# plot(lasso.glm)
# 
...
# bestlam.lasso # 0.000938566
# 
# # Predict using the best lambda
# lasso.pred <- predict(lasso.glm, s = bestlam.lasso, newx = x[test,])
# mse.lasso <- mean((lasso.pred - y.test)^2) # 0.1173978
# 
# # What variables should we take out?
# predict(lasso.glm, type = "coef", s = bestlam.lasso)
